...
/**/			
    ...
    private static function outputProcessingFiles($layoutId,$layStatusId,$layEdition,$jobname,$fullrow,$mtparr){
        ...
        $scriptText.='var myPreset=app.pdfExportPresets.item("'.$jobname.'");if(myPreset==null){try{var presetFile="'.MYMTP_IDS_PRESETS_PATH.$jobname.'.joboptions";app.importFile(ExportPresetFormat.pdfExportPresetsFormat,presetFile);myPreset=app.pdfExportPresets.item("'.$jobname.'");}catch(err){app.performSimpleRequest("'.$requestUrl.'5&message=cannot%20load%20preset%20'.$jobname.'");myDoc.close(SaveOptions.no);exit(0);}}';
