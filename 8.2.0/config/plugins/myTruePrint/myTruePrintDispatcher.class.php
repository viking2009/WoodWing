...
class myTruePrintDispatcher
{
    ...
    private static function outputProcessingFiles($layoutId,$layStatusId,$layEdition,$jobname,$fullrow,$mtparr){
        ...
        $scriptText='try{app.scriptPreferences.userInteractionLevel=UserInteractionLevels.neverInteract;}catch(e){}try{app.entSession.login("'.MYTP_USER.'","'.MYTP_PASSWORD.'","'.MYTP_SERVER_DEF_ID.'");}catch(err){app.performSimpleRequest("'.$requestUrl.'2&message=cannot%20logon%20MtP%20user");exit(0);}try{myDoc=app.openObject("'.$layoutId.'",false);}catch(err){app.performSimpleRequest("'.$requestUrl.'3&message=cannot%20open%20layout%20'.$layoutId.'");app.entSession.logout();exit(0);}';
        if ($layEditionId>0) {
