...
//		require_once BASEDIR.'/server/bizclasses/BizObjectJob.class.php'; // v8.0: Uncomment when serverJob 'UpdateParentModifierAndModified' is supported again.
	 ...
	 * @return Object
	 */
	public static function getObject( $id, $user, $lock, $rendition, $requestInfo = null, $haveVersion = null, $checkRights = true, 
										array $areas=null, $editionId = null )
	{
		...
		try {
			// get the object
			$objectProps = BizQuery::queryObjectRow($id, $areas );
	...
	public static function setObjectProperties( $id, $user, MetaData $meta, $targets )
	{
		...
		if( $meta->BasicMetaData->Type == 'Dossier' ) {
	...
	public static function copyObject( $srcid, $meta, $user, $targets, $pages, $relations=null )
	{
			...
			for( $i = 0; $i < count($pages); $i++ ) {
				...
				for ($nr=1; $nr <= $prow['nr']; $nr++)
				{
					...
					foreach (unserialize($prow['types']) as $tp) {
						...
						$dummy = null;
						if( !$pageobj->copyFile( $newVerNr, $id, $storename, "$iPageOrder-$nr", null, $dummy, null ) ) {
							throw new BizException( 'ERR_ATTACHMENT', 'Server', $pageobj->getError() );
						}
					}
				}
		...
		// ===> 5a. Create a new dossier for this newly copied object or assign it to a selected dossier (If applicable)
		...
		// 8. Copy object sticky note message
		require_once BASEDIR.'/server/bizclasses/BizMessage.class.php';
		...
		if( $autonaming !== true ) { // ignore this rule when we'll make up the name ourself
			...
			if( !$id || isset($uniqueIssueTypes[$type])) { // only for new objects; create and copy or for special "print" types (BZ#9554)
				...
				if( count($issueIds) > 0 ) { // no targets, no validation
					require_once BASEDIR.'/server/dbclasses/DBObject.class.php';
					if( self::objectNameExists( $pub, $issueIds, $name, $type, $id ) ) {
	 ...
	 *
	 * @param string $pub Publication id
	 ...
	 */
	public static function objectNameExists($pub, $issueIds, $name, $type, $id=null)
	{
		require_once BASEDIR.'/server/dbclasses/DBObject.class.php';
		return DBObject::objectNameExists( $pub, $issueIds, $name, $type, $id );
	}
	...
	{
		// 1. Enrich object MetaData with any embedded metadata from file
		require_once BASEDIR.'/server/bizclasses/BizMetaDataPreview.class.php';
		$bizMetaPreview = new BizMetaDataPreview();
		$bizMetaPreview->readMetaData( $object );
		$bizMetaPreview->generatePreviewNow( $object );

		// 2. Validate meta data and targets
		$arr = self::validateMetaDataAndTargets( $user, $object->MetaData, $object->Targets, false, $autonaming );
	...
	private static function saveExtended( $id, Object $object, $user, $create )
	{
		$dbDriver = DBDriverFactory::gen();
	...
	static protected function createDossierFromObject($object)
	{
		...
		if ($state){
			...
			// is created for the newly created Dossier. This is a hackish sollution but the only way to get the info in
			// there at this moment.
			if ($object->Relations)	foreach ($object->Relations as $relation) {
				if ($relation->Parent === '-1' && $object->MetaData->BasicMetaData->Type === 'Article') {
	...
	public static function updateParentObject( $job )
	{
		...
		try{
			if( $objName ){ // objectName found indicates this object still exists in the system.
				...
				// Retrieve the object from workflow / Trash depending on where this object residing.
	 ...
	 * @param dateTime $modified Timestamp. 
	 */
